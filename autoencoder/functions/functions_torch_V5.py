...
class FairAutoEncoder(torch.nn.Module):
    ...
    def __init__(self, n_inputs, n_clusters, activation):
...
class LeNet_300_100(nn.Module):
    def __init__(self, n_inputs, n_outputs=2, activation= "relu"):
...
class netBio(nn.Module):
    def __init__(self, n_inputs, n_outputs=2, n_hidden=300, activation="relu"):
    ...
    for epoch_idx in range(N_EPOCHS):
        ...
        if run_model == "ProjectionLastEpoch" and epoch_idx == (N_EPOCHS - 1):
            ...
            for index, param in enumerate(net_parameters):
                ...
                #       )
                if(( DO_PROJ_MIDDLE == True or not is_middle ) and ( not is_decoder_layer or DO_PROJ_DECODER )):
                    if is_decoder_layer:
                        param.data = Projection(
                            param.data.T, TYPE_PROJ, ETA, RHO=RHO, AXIS=AXIS, ETA_STAR=ETA_STAR, device=device, TOL=TOL,).to(device).T
                    else:
                        param.data = Projection(
                            param.data, TYPE_PROJ, ETA, RHO=RHO, AXIS=AXIS, ETA_STAR=ETA_STAR, device=device, TOL=TOL,).to(device)
...
# =============================================================================
    ...
    title = f"Entropy vs Epoch ({'Proj' if run_model == 'MaskGrad' else 'Initial'}, Training, Seed: {seed} in {SEEDS}, Fold {fold_idx+1}/{nfolds})"
    ...
    #print(f"\nFOR EVERY EPOCH {epoch_val_loss}\n")    
    title = f"ReconLoss vs Epoch ({'Proj' if run_model == 'MaskGrad' else 'Initial'}, Training, Seed: {seed} in {SEEDS}, Fold {fold_idx+1}/{nfolds})"
    y_data = epoch_reconstruction[:-1]
    y_label = "ReconLoss"
    x_data = range(0, len(y_data))
    x_label = "Epoch"
    plotGraph(y_data, x_data, y_label, x_label, title)
    ...
    #print(f"\nFOR EVERY EPOCH {epoch_val_loss}\n")
    title = f"Entropy vs Epoch ({'Proj' if run_model == 'MaskGrad' else 'Initial'}, Validation, Seed: {seed} in {SEEDS}, Fold {fold_idx+1}/{nfolds})"
    ...
    #print(f"\nFOR EVERY EPOCH {epoch_val_loss}\n")    
    title = f"ReconLoss vs Epoch ({'Proj' if run_model == 'MaskGrad' else 'Initial'}, Validation, Seed: {seed} in {SEEDS}, Fold {fold_idx+1}/{nfolds})"
    y_data = epoch_val_reconstruction[:-1]
    y_label = "ReconLoss"
    x_data = range(0, len(y_data))
    x_label = "Epoch"
    plotGraph(y_data, x_data, y_label, x_label, title)
...
def Projection(
    W, TYPE_PROJ=proj_l11ball, ETA=100, RHO=0.5, AXIS=0, ETA_STAR=100, device="cpu", TOL=1e-3
):
    ...
    if TYPE_PROJ == proj_l1Inftyball_line_unbounded:
        W_new = TYPE_PROJ(W, RHO, device=device)  
...
def topGenes(
    X, Y, feature_name, class_len, feature_len, method, nb_samples, device, net, tol=1e-3
):
